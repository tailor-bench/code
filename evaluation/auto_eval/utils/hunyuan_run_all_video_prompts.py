...
DEFAULT_OUT_DIR = os.path.join(SCRIPT_DIR, "results", "hunyuan_video")
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Run HunyuanVideo on all predictive/descriptive video prompts from selected_tasks_with_prompts.json"
    )
    ...
    p.add_argument("--device", default="cuda:0", help="Device for generation (e.g. cuda:0)")
...
def load_pipeline(device: str, dtype=torch.bfloat16):
    pipe = HunyuanVideo15Pipeline.from_pretrained(MODEL_ID, torch_dtype=dtype)
    pipe.enable_model_cpu_offload(device=device)
    pipe.vae.enable_tiling()
    return pipe


def generate_video(pipe, prompt: str, out_path: str, device: str, seed: int) -> None:
...
def main():
    ...
    print("Loading HunyuanVideo pipeline...", flush=True)
    ...
    for task_idx, task in enumerate(tasks):
        ...
        for inst_idx, inst in enumerate(instances):
            ...
            for prompt_type, key in [
                ...
                try:
                    generate_video(pipe, prompt, out_path, args.device, seed)
